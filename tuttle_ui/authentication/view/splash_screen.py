...
class SplashScreen(AuthView):
    def __init__(self, changeRouteCallback: Callable[[str, typing.Optional[any]], None], localCacheHandler : LocalCache):
        super().__init__(changeRouteCallback=changeRouteCallback, localCacheHandler=localCacheHandler)
    
    def on_log_in_clicked(self, title :str, name : str, email : str, phone : str, address : str):
        return self.intentHandler.create_user(title=title, name=name,email=email,phone=phone, address=address)

    def on_logged_in(self, data : typing.Optional[any]=None,):
    ...
    def show_login_form(self):
        form = LoginForm(onLoggedIn = self.on_logged_in, onLogInClicked=self.on_log_in_clicked,)
