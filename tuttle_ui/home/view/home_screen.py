...
from flet import (Column, Container, NavigationRailDestination, IconButton, NavigationRail, Row, UserControl,
                  VerticalDivider, icons, padding)

from core.abstractions.local_cache import LocalCache
from core.abstractions.view import TuttleView
from core.ui.utils.flet_constants import (CENTER_ALIGNMENT, CENTER_RAIL, START_ALIGNMENT,
                                          COMPACT_RAIL_WIDTH, NEVER_SHOW,
...
class HomeScreen(TuttleView, UserControl):
    def __init__(self, changeRouteCallback:Callable[[str, typing.Optional[any]], None], localCacheHandler : LocalCache):
        super().__init__(hasFloatingActionBtn=False, hasAppBar=True, onChangeRouteCallback=changeRouteCallback)
        ...
        self.destination_body = Column( controls=[
            initialDestination
        ], alignment=START_ALIGNMENT, horizontal_alignment=START_ALIGNMENT, expand=True)
    ...
    def build(self):
        ...
        page_view = Row(
            [
                ...
                VerticalDivider(width=spacing.SPACE_XS),
                Container(
                    expand = True,
                    bgcolor = colors.SILVER_GRAY_COLOR,
